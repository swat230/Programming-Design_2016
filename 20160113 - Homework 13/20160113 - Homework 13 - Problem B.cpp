...
{
    private :
    static int offlineCnt_P ; //define a static int for child
...
{
    private :
    static int offlineCnt_D ;//define a static int for child
...
int main()
{
    ...
    while (cin >> type)
    {
        if (type == 'P')//add a new passenger
        {
            int id = 0,level = 0;
            cin >> level >> id ;
            p[p_count] = new Passenger(level,id);
            p_count++;
        }
        ...
        if (type == 'S')//passenger search for driver
        {
            ...
            for(int i = 0; i < p_count; i++)
            {
                ...
                {
                    if(p[i]->level == 0)//if the passenger is normal user
                    {
                    ...
                    if (p[i]->level == 1)//if the passenger is vip
                    {
                        if(Driver::get_search() > 0)//make sure if there is any available driver
                        {
                            int point = 0;
                            cin >> point;//input the picked driver
                            p[i]->driver_id = point;
                            for (int j = 0; j < d_count; j++)
                            {
                                if (d[j]->get_id() == point)
